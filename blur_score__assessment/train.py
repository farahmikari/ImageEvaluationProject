...
class Config:
    TRAIN_DIR: str = "./DIV2K_train_HR"
    TEST_DIR: str = "./DIV2K_test_HR"
    MODEL_SAVE_PATH: str = "./best_blur_model_efficientnet.pth"
...
def motion_blur_rgb(image_rgb: np.ndarray, sigma: float) -> np.ndarray:
    if sigma <= 0:
        return image_rgb
    length = int(np.clip(round(2.4 * sigma + 3), 3, 51))
    length = length | 1
...
def defocus_blur_rgb(image_rgb: np.ndarray, sigma: float) -> np.ndarray:
    if sigma <= 0:
        return image_rgb
    radius = int(np.clip(round(0.9 * sigma), 1, 18))
    size = radius * 2 + 1
...
def apply_synthetic_blur(image_rgb: np.ndarray, sigma: float, training: bool) -> Tuple[np.ndarray, str]:
    if sigma <= 0:
        return image_rgb, "none"

    if not training:
        return gaussian_blur_rgb(image_rgb, sigma), "gaussian"

    # Severe real images are often not pure Gaussian. Add motion/defocus mainly
    # in the high range so the top scores do not depend on one blur shape only.
    if sigma >= 5.5:
...
class SmartBlurDataset(Dataset):
    ...
    def __getitem__(self, idx: int) -> Tuple[torch.Tensor, torch.Tensor]:
        ...
        patch, _ = apply_synthetic_blur(patch, sigma, training=self.training)
...
def evaluate_model(model: nn.Module, loader: DataLoader, name: str, print_samples: bool = False) -> Tuple[float, float]:
    ...
    if print_samples:
        print(f"  first predictions %: {np.round(target_to_percent(preds_np[:12]), 2)}")
        print(f"  first targets     %: {np.round(target_to_percent(labels_np[:12]), 2)}")
